...
window.onload = function () {
    document.getElementById("testa").innerHTML = "update2";
    document.getElementById("btntest").onclick = function (evt) {
        addimage()
    }
} 

//document.getElementById("btntest").addEventListener("click", addimage, false);  
//document.getElementById("myDiv").innerHTML = "<img src='images/bussiness/caffe2.jpg'>";
   ...
   //console.log(item.name);
                //console.log(item.name.substr(item.name.lastIndexOf('.'), 4)); 



    //var fileextension = ".svg"; 

    //var businessDir = "images/business";
    //$.ajax({
    //    url: businessDir,
    ...
    //        $('body').html(text_status + ': ' + error_thrown);
    //    }
    //});
...
function addimage() { 
        ...
        var path = root + "\\images\\bussiness";
            ...
            filesInFolderPromise.done(function getFilesSuccess(filesInFolder) {
                filesInFolder.forEach(function forEachFile(item) {             
                    if (item.name.includes("svg")) {
                        ...
                        img.src = "images/bussiness/" + item.name;
